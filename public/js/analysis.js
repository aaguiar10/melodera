...
function getFeaturedPlists() {
  let date = new Date()
  let timestampISO = new Date(date.getTime() - (date.getTimezoneOffset() * 60000)).toISOString()
  return fetch('/featuredPlists?offset=' + defaultOset + 
        '&user_country=' + profileInfo['userCountry'] + 
      ...
      var featPSection =
        $(`<div class="container" id="featPlistsContainer">
        ...
        featPlistDiv.innerHTML =
                `<div>
                  ...
                  ${playlist.description.trim() !== "" ? 
                    ...
                    </ol>`: `<ol class="addResultText" id=${playlist.id}></ol>`}
                 </div>`
...
function getNewReleases() {
  fetch('/newReleases?offset=' + defaultOset + 
        '&user_country=' + profileInfo['userCountry'],
       {
        method: "GET",
        headers: {
      ...
      var newRelSection =
        $(`<div class="container" id="newRelsContainer">
            <button class='btn btn-lg rounded-pill searchCateg activeCateg' 
        ...
        newRelDiv.innerHTML =
                `<div>
                  <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                       ...
                       width="64" height="64">
                  <p class="resultText">
                    ${(album.artists.map((eachArtist) => eachArtist.name).join(', '))} - ${album.name}
                  </p>
                  ${album.release_date.trim() !== "" ? 
                    ...
                    </ol>`: `<ol class="addResultText" id=${album.id}></ol>`}
                 </div>`
...
async function loadFunctions() {
  ...
  if (profileInfo['subLevel'] === "open") {
    ...
    if (alerted != 'yes') {
     alert(`Spotify Premium is required to control Melodera's audio player / visualizer.`+
    ...
    .then((data) => {
      ...
      if (profileInfo['subLevel'] === "open" && 
          ...
          getFeatures(data.item.id);
              ...
              !data.is_playing && data.item.id === currentTrack) {
...
function getTopArtists(timeframe) {
  checkExpir();
  if (document.getElementById("results").contains(document.getElementById(`artist_${timeframe}`))) {
    document.getElementById(`artist_${timeframe}`).scrollIntoView();
    return;
  }
  
  fetch('/topArtists?time_range=' + timeframe + 
        '&offset=' + defaultOset,
       {
        method: "GET",
        headers: {
      ...
      if (document.getElementById("results").contains(document.getElementById('btnTracksCateg')) ||
          document.getElementById("results").contains(document.getElementById('btnFeatPlistsCateg'))) 
      {
        document.getElementById("results").innerHTML = "";
      }
      var topArtistsDiv = document.createElement('div');
      topArtistsDiv.setAttribute('id', `topArtistsCateg_${timeframe}`);
      if(document.getElementById("results").contains(document.getElementById(`btnTopArtistsCateg`))) {
        var currTarget = document.getElementById(`btnTopArtistsCateg`).getAttribute('data-bs-target');
        var currAria = document.getElementById(`btnTopArtistsCateg`).getAttribute('aria-controls');
        var currExpand = document.getElementById(`btnTopArtistsCateg`).getAttribute('aria-expanded');
        document.getElementById(`btnTopArtistsCateg`).setAttribute('data-bs-target', currTarget + 
                                                                  `,#topArtistsCateg_${timeframe}, 
                                                                    #moreBtnTopArtists_${timeframe},
                                                                    #artist_${timeframe}`);
        document.getElementById(`btnTopArtistsCateg`).setAttribute('aria-controls', currAria + 
                                                                  `,topArtistsCateg_${timeframe}, 
                                                                    moreBtnTopArtists_${timeframe},
                                                                    artist_${timeframe}`);
        if (currExpand==='false') {
            document.getElementById(`btnTopArtistsCateg`).setAttribute('aria-expanded', 'true');
            var xvv = document.getElementById('topArtistsContainer').querySelectorAll(".collapse");
            xvv.forEach(function (elem) {
              elem.classList.toggle('show');
            })
            document.getElementById('btnTopArtistsCateg').classList.toggle('activeCateg');
        }
        var topASection = 
          $(`<h4 class="collapse show tFrametitle" 
      ...
      var topASection =
        $(`<div class="container" id="topArtistsContainer">
            <button class='btn btn-lg rounded-pill searchCateg activeCateg' 
      ...
      data.items.forEach(function(artist, index) {
        ...
        artistDiv.setAttribute('onclick', `getArtistSongs('${artist.id}', this);`);
        artistDiv.innerHTML =
                `<div>
                  <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                       src="${artist.images.length === 0 
                              ? 'images/music-note-beamed.svg'
                              : artist.images[2].url}"
                       width="64" height="64">
                  <p class="resultText">
      ...
      document.getElementById('topArtistsContainer').insertBefore(topArtistsDiv, 
...
document.getElementById("circVisual").addEventListener("click", function(event) {
    localStorage.setItem("beat_visualizer_state", JSON.stringify(true));
    localStorage.setItem("beat_visualizer_type", "circle");
    location.reload();
});
document.getElementById("triVisual").addEventListener("click", function(event) {
    localStorage.setItem("beat_visualizer_state", JSON.stringify(true));
...
document.getElementById("mixVisual").addEventListener("click", function(event) {
    localStorage.setItem("beat_visualizer_state", JSON.stringify(true));
...
function getTopTracks(timeframe) {
  checkExpir();
  if (document.getElementById("results").contains(document.getElementById(`track_${timeframe}`))) {
    document.getElementById(`track_${timeframe}`).scrollIntoView();
    return;
  }
  
  fetch('/topTracks?time_range=' + timeframe + 
        '&offset=' + defaultOset,
       {
        method: "GET",
        headers: {
      ...
      if (document.getElementById("results").contains(document.getElementById(`btnTopTracksCateg`))) {
        var currTarget = document.getElementById(`btnTopTracksCateg`).getAttribute('data-bs-target');
        var currAria = document.getElementById(`btnTopTracksCateg`).getAttribute('aria-controls');
        var currExpand = document.getElementById(`btnTopTracksCateg`).getAttribute('aria-expanded');
        document.getElementById(`btnTopTracksCateg`).setAttribute('data-bs-target', currTarget + 
                                                                  `,#topTracksCateg_${timeframe}, 
                                                                    #moreBtnTopTracks_${timeframe},
                                                                    #track_${timeframe}`);
        document.getElementById(`btnTopTracksCateg`).setAttribute('aria-controls', currAria + 
                                                                  `,topTracksCateg_${timeframe}, 
                                                                    moreBtnTopTracks_${timeframe},
                                                                    track_${timeframe}`);
        if (currExpand==='false') {
            document.getElementById(`btnTopTracksCateg`).setAttribute('aria-expanded', 'true');
            var xvv = document.getElementById('topTracksContainer').querySelectorAll(".collapse");
            xvv.forEach(function (elem) {
              elem.classList.toggle('show');
            })
            document.getElementById('btnTopTracksCateg').classList.toggle('activeCateg');
        }
        var topTSection = 
          $(`<h4 class="collapse show tFrametitle" 
      ...
      var topTSection =
        $(`<div class="container" id="topTracksContainer">
             <button class='btn btn-lg rounded-pill searchCateg activeCateg' 
      ...
      data.items.forEach(function(track, index) {
        ...
        trackDiv.setAttribute('onclick', `playVid('${track.id}');
        ...
        trackDiv.innerHTML = 
                `<div>
                  <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                       ...
                       width="64" height="64">
                  <p class="resultText">
                    ${index+1}.  ${track.artists.map((eachArtist) => eachArtist.name).join(', ')} - ${track.name}
                  </p>
      ...
      document.getElementById('topTracksContainer').insertBefore(topTracksDiv, 
...
function drawAnalysis(data) {
  ...
  featuresChart.addEventListener("click", (clickEvent) => {
    if (profileInfo['subLevel'] === "open") {
      alert(`Spotify Premium is required to control Melodera audio player / visualizer.`+
    ...
    if (pitchesObj['startTime'].length !== 0) {
      ...
      else if (pitchesObj['pitch'][pitchCounter] === null || !(Number.isInteger(pitchCounter))) {
        document.getElementById("domPitch").innerHTML = "unknown";
        pitchCounter++;
      }
      else {
        document.getElementById("domPitch").innerHTML = pitchesObj['pitch'][pitchCounter];
        pitchCounter++;
      }
    }
    if (beatsObj['startTime'].length !== 0) {
      ...
      sVisualCtx.fillStyle = colors[pitchesObj['pitchSectionI'][pitchCounter - 1] % colors.length];
      ...
      if (beatCounter == -1) {
        beatCounter = 0;
        prevBeatCounter = 0;
        getVisType(sVisualCtx,
                   easeInOutSine(seekTime, 
  ...
  arrayLikes.forEach((arrayLike, arrayLikeIndex) => {
    ...
    arrayLike.forEach((section, sectionIndex) => {
      ...
      if (arrayLikesKeys[arrayLikeIndex] == "segments" && section.confidence >= 0.75) {
    ...
    if (profileInfo['subLevel'] === "open") {
      fetch("https://api.spotify.com/v1/me/player/currently-playing", {
        ...
        .then((data) => {
          ...
          document.getElementById("timer").innerHTML =
            (((data.progress_ms % 60000) / 1000).toFixed(0) == 60
              ...
              : Math.floor(data.progress_ms / 60000) +
                ":" +
                (((data.progress_ms % 60000) / 1000).toFixed(0) < 10 ? "0" : "") +
          ...
          const markPosition =
            (data.progress_ms / 1000 / (data.item.duration_ms / 1000)) * width;
          if (pitchesObj['startTime'].length !== 0 && currPosition > 0 && pitchCounter) {
            const seekPitch = binaryIndexOf.call(
              pitchesObj["startTime"],
              currPosition,
              (e) => e,
          ...
          if (beatCounter !== (beatsObj['startTime'].length) &&
              Math.abs(currPosition - beatsObj['startTime'][beatCounter]) < 0.5) {
            if (pitchCounter !== (pitchesObj['startTime'].length)) {
              if (pitchesObj['startTime'][pitchCounter] === null || !(Number.isInteger(pitchCounter))) {
                document.getElementById("domPitch").innerHTML = "unknown";
                pitchCounter++;
              }
              else if (Math.fround(currPosition) > 
            ...
            if (Math.fround(currPosition) > Math.fround(beatsObj['startTime'][beatCounter])) {
          ...
          document.getElementById("timer").innerHTML =
            (((state.position % 60000) / 1000).toFixed(0) == 60
              ...
              : Math.floor(state.position / 60000) +
                ":" +
                (((state.position % 60000) / 1000).toFixed(0) < 10 ? "0" : "") +
...
function getArtistSongs(artistId, divClicked) {
 if (divClicked.getElementsByClassName('addResultText')[0].textContent.trim() === "") {
  fetch("https://api.spotify.com/v1/artists/" + artistId + "/top-tracks?market=" + 
    ...
    .then((data) => {
      divClicked.getElementsByClassName('addResultText')[0].style.display="block";
      divClicked.getElementsByClassName('addResultText')[0].style.maxHeight="150px";
    ...
    if (divClicked.getElementsByClassName('addResultText')[0].style.display==="block") {
        divClicked.getElementsByClassName('addResultText')[0].style.display="none";
    }
    else divClicked.getElementsByClassName('addResultText')[0].style.display="block";
  }
  ...
  fetch("https://api.spotify.com/v1/albums/" + albumId + "/tracks?limit=50", {
    ...
    .then((data) => {
      ...
      if (data.items.length === 0) {
        document.getElementById(albumId).innerHTML = 
          `<p>
               &lt;Nothing found&gt;
          </p>`
      } else {
...
function getPlistSongs(plistId) {
 if (document.getElementById(plistId).textContent.trim() === "") {
  fetch("https://api.spotify.com/v1/playlists/" + plistId + "/tracks?limit=50", {
    headers: {
      Authorization: `Bearer ${accessToken}`,
    },
  }).then((e) => e.json())
    .then((data) => {
      ...
      if (data.items.length === 0) {
        document.getElementById(plistId).innerHTML = 
          `<p>
               &lt;Nothing found&gt;
          </p>`
      } else {
        ...
        document.getElementById(plistId).innerHTML =
        data.items.filter(result => result.track !== null).map(
                (result) =>
                  `<li onClick="playVid(&apos;${result.track.id}&apos;); 
                    getAnalysis(&apos;${result.track.id}&apos;); getFeatures(&apos;${result.track.id}&apos;);">
                    ${result.track.artists.map((eachArtist) => eachArtist.name).join(', ')} - ${result.track.name}
                   </li>`
                ).join("")
      }
  ...
  if (profileInfo['subLevel'] === "open") {
    return fetch("https://api.spotify.com/v1/me/player/currently-playing", {
      ...
      .catch((error) => {
        ...
        if (alerted != 'yes') {
          alert(`Choose song through another device (desktop client, phone, etc.) and`+
  ...
  if (isLibrary) {
    if (category === "tracksCateg") {
      ...
      if (tracksOffset > 100) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/me/tracks?limit=4&offset='
                + tracksOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
      ...
      if (artistsOffset > 100) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/me/following?type=artist&limit=4&after='
                + afterArtistOset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
      ...
      if (albumsOffset > 100) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/me/albums?limit=4&offset='
                + albumsOffset + 
                '&market=' + profileInfo['userCountry'], {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
      ...
      if (plistsOffset > 100) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/me/playlists?limit=4&offset='
                + plistsOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
    ...
    if (category === "tracksCateg") {
      ...
      if (tracksOffset > 50) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/search?q=' 
                + searchVal + '&type=track&limit=4&offset='
                + tracksOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
      ...
      if (artistsOffset > 50) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/search?q=' 
                + searchVal + '&type=artist&limit=4&offset='
                + artistsOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
      ...
      if (albumsOffset > 50) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/search?q=' 
                + searchVal + '&type=album&limit=4&offset='
                + albumsOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
                    indivResult['albums'] = result;
      ...
      if (plistsOffset > 50) alert("Limit reached")
        ...
        Promise.all([
          fetch('https://api.spotify.com/v1/search?q=' 
                + searchVal + '&type=playlist&limit=4&offset='
                + plistsOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
          .then(resp => resp.json()),
        ]).then((data) => {
                data.map(function(result) {
      ...
      fetch('/topTracks?time_range=' + category.substring(15) + 
            '&offset=' + topTracksOffset[category.substring(15)],
         {
          method: "GET",
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      fetch('/topArtists?time_range=' + category.substring(16) + 
            '&offset=' + topArtistsOffset[category.substring(16)],
         {
          method: "GET",
          headers: {
            Authorization: `Bearer ${accessToken}`,
...
function showResults(data, isLibrary) {
  document.getElementById("results").innerHTML =
          ...
          data.tracks.items
            .map(
              (result) =>
                ...
                onClick="playVid(&apos;${isLibrary ? result.track.id : result.id}&apos;); 
                getAnalysis(&apos;${isLibrary ? result.track.id : result.id}&apos;); 
                         getFeatures(&apos;${isLibrary ? result.track.id : result.id}&apos;);"
                >
                  <div>
                    <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                         src="${isLibrary ? (result.track.album.images.length === 0 ? 
                                             'images/music-note-beamed.svg': 
                         ...
                         width="64" height="64">
                    <p class="resultText">
                    ${isLibrary ? (result.track.artists.map((eachArtist) => eachArtist.name).join(', ')) : 
                      (result.artists.map((eachArtist) => eachArtist.name).join(', '))} - ${isLibrary ? result.track.name : result.name}
                    </p>
                    ${isLibrary ? 
          ...
          data.albums.items
            .map(
              (result) =>
                `<div class="text-white"
                onClick="getAlbumSongs(&apos;${isLibrary ? result.album.id : result.id}&apos;);"
                >
                  <div>
                    <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                         src="${isLibrary ? (result.album.images.length === 0 ? 
                                             'images/music-note-beamed.svg': 
                                             result.album.images[2].url) : 
                         ...
                         width="64" height="64">
                    <p class="resultText">
                    ${isLibrary ? (result.album.artists.map((eachArtist) => eachArtist.name).join(', ')) : 
                      (result.artists.map((eachArtist) => eachArtist.name).join(', '))} - ${isLibrary ? result.album.name : result.name}
                    </p>
            ...
            .map(
              (result) =>
                ...
                >
                  <div>
                    <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                         src="${result.images.length === 0 ? 'images/music-note-beamed.svg': result.images[0].url}"
                         width="64" height="64">
                    <p class="resultText">
  ...
  if (data.artists.items.length > 0) afterArtistOset = data.artists.items[data.artists.items.length - 1].id
}
            ...
            .map(
              ...
              `<div class="text-white-top"
                ...
                >
                  <div>
                    <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                         src="${result.album.images.length === 0 ?
                                'images/music-note-beamed.svg': 
                                result.album.images[2].url}"
                         width="64" height="64">
                    <p class="resultText">
                    ${oSet++}.  ${result.artists.map((eachArtist) => eachArtist.name).join(', ')} - ${result.name}
                    </p>
                    ${result.album.release_date.trim() !== "" ? 
                      `<p class="resultText fw-light fs-6">Released: ${result.album.release_date}</p>
                      </div>`: `</div>`}
              ...
              `<div class="text-white-top"
                ...
                >
                  <div>
                    ...
                    <ol class="addResultText" id=${result.id + category.substring(15)}>
                    </ol>
                  </div>
               </div>`
            )
...
function getMyLibrary() {
  ...
  Promise.all([
    fetch('https://api.spotify.com/v1/me/tracks?limit=4&offset='
          + tracksOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
    .then(resp => resp.json()),
    fetch('https://api.spotify.com/v1/me/following?type=artist&limit=4&offset='
          + artistsOffset, {headers: {Authorization: `Bearer ${accessToken}`}})
    .then(resp => resp.json()),
    fetch('https://api.spotify.com/v1/me/albums?limit=4&offset='
          + albumsOffset + 
  ...
  return fetch("https://api.spotify.com/v1/audio-features/" + id, {
    ...
    .then((data) => {
      ...
      document.getElementById("bpmAndKey").innerHTML =
        "Key: " +
        `<span id='keyOfT'>${data.key} ${data.mode == 1 ? 'Major': 'Minor'}</span>` + 
  ...
  return fetch("https://api.spotify.com/v1/tracks/" + id, {
    ...
    .then((data) => {
      ...
      .then((data) => {
        ...
        document.getElementById("features-chart-container").style.backgroundColor =
          "rgba(" + rgbArray1[0] + ", " + rgbArray1[1] + ", " + rgbArray1[2] + ", 0.3)";
        document.getElementById("features-chart-container").style.border = "2px solid black";
        ...
        document.getElementById("cover_and_play").style.border = 
          "5px solid rgb(" + rgbArray2[0] + ", " + rgbArray2[1] + ", " + rgbArray2[2] +")";
  ...
  player.on("player_state_changed", (state) => {
      ...
      ) 
    {
      ...
      prevBeatCounter = 0;
             ...
             window.getComputedStyle(document.getElementById('play')).display !== "none") {
      pauseVidAuto();
    }
    else if (this.state && !state.paused && 
             window.getComputedStyle(document.getElementById('play')).display !== "none") {
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.querySelector("form").addEventListener("submit", function (event) {
    ...
    checkExpir();
    searchVal = (input.value.trim() === "" ? "johann sebastian bach" : input.value)
    const searchQuery =
      "/search?query=" + searchVal + "&offset=" + defaultOset;
    tracksOffset = 0;
    artistsOffset = 0;
    albumsOffset = 0;
    plistsOffset = 0;
...
function showSimilar() {
  fetch(
    ...
    .then((e) => e.json())
    .then((data) => {
      if (document.body.contains(document.getElementById('simTrackCateg')) && showSimLimit > 100) {
        alert("Limit reached");
      }
      else if (document.body.contains(document.getElementById('simTrackCateg'))) {
        document.getElementById('simTrackCateg').innerHTML =
          data.tracks.map(
            (track) =>
            `<div class="text-white"
              ...
              >
                <div>
                  <img style="padding-top:10px; display: flex; margin: 0px auto 0px auto; width: 64px; height:auto;" 
                       ...
                       width="64" height="64">
                  <p class="resultText">
                    ${track.artists.map((eachArtist) => eachArtist.name).join(', ')} - ${track.name}
                  </p>
                </div>
             </div>`
          )
          .join("");
        document.querySelector("button[aria-controls='simTrackCateg, moreBtnSimT']").setAttribute('aria-expanded','true');
        document.getElementById('simTrackCateg').classList = 'collapse show searched';
        document.getElementById('moreBtnSimT').classList = 'collapse show btn moreBtn';
        showSimLimit += 4;
        document.getElementById("simTrackCateg").scrollIntoView();
      } 
      else{
        var simTrackElem1 = document.createElement("button")
        simTrackElem1.setAttribute('class', 'btn btn-lg rounded-pill searchCateg activeCateg');
        ...
        simTrackElem1.setAttribute('data-bs-target', '#simTrackCateg, #moreBtnSimT');
        simTrackElem1.setAttribute('onclick', "toggleCateg('btnSimT')");
        simTrackElem1.setAttribute('aria-expanded', true);
        simTrackElem1.setAttribute('aria-controls', 'simTrackCateg, moreBtnSimT');
...
function popPlist() {
  ...
  fetch("https://api.spotify.com/v1/me/playlists?limit=50", {
    ...
    .then((data) => {
      if (data.items.length === 0) {
        document.getElementById("plistOptions").innerHTML = 
          `<p style="margin: auto">
               &lt;Nothing found&gt;
          </p>`
      } 
        ...
        data.items.map(function(result) {
          if (result.owner.id === profileInfo['currentUser'] || result.collaborative === true) {
            ...
            plistElemBtn.setAttribute('onclick', 
...
window.onkeydown= function(event){
      if (document.activeElement !== document.getElementById("inputSrch") &&
          document.getElementById("displaysong").contains(document.getElementById("trackStatus"))) {
        if(event.keyCode === 32 &&
           document.getElementById("playedButton").style.display === "block"){
          resumeVid();
        }
