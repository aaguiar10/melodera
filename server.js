...
app.get("/search", function (request, response) {
  reAuthenticateOnFailure((failure) => {
    spotifyApi   
      .searchTracks(request.query.query, { limit: 4,
                                          offset: request.query.offset })
      .then(function (data) {
       o1 = data.body;
       spotifyApi.searchArtists(request.query.query, { limit: 4,
                                                      offset: request.query.offset })
       .then(function (data) {
         o2 = data.body;
         spotifyApi.searchAlbums(request.query.query, { limit: 4,
                                                       offset: request.query.offset })
         .then(function (data) {
           o3 = data.body;
           spotifyApi.searchPlaylists(request.query.query, { limit: 4,
                                                            offset: request.query.offset})
           .then(function (data) {
             o4 = data.body;
...
app.get("/topArtists", function (request, response) {
  ...
  reAuthenticateOnFailure((failure) => {
    spotifyApi.getMyTopArtists({time_range:request.query.time_range, limit: 10, 
                                offset: request.query.offset})
      .then(function(data) {
        response.send(data.body);
      }, failure);
...
app.get("/topTracks", function (request, response) {
  ...
  spotifyApi.setAccessToken(request.headers['authorization'].split(' ')[1]);
  reAuthenticateOnFailure((failure) => {
    spotifyApi.getMyTopTracks({time_range:request.query.time_range, limit: 10, 
                               offset: request.query.offset})
      .then(function(data) {
        response.send(data.body);
      }, failure);
...
app.get("/featuredPlists", function (request, response) {
  ...
  spotifyApi.setAccessToken(request.headers['authorization'].split(' ')[1]);
  reAuthenticateOnFailure((failure) => {
    spotifyApi.getFeaturedPlaylists({ limit : 12, 
                                      offset: request.query.offset, 
...
app.get("/newReleases", function (request, response) {
  ...
  spotifyApi.setAccessToken(request.headers['authorization'].split(' ')[1]);
  reAuthenticateOnFailure((failure) => {
    spotifyApi.getNewReleases({ limit : 12, 
                                offset: request.query.offset, 
                                country: request.query.user_country,
                              })
      .then(function(data) {
